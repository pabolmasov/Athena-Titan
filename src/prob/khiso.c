...
void problem(DomainS *pDomain)
{
  ...
  Real amp,drat,vflow,b0,a,sigma,x1,x2,x3, permode;
  long int iseed = -1;
...
/* Read problem parameters */
  ...
  permode = par_getd("problem","permode"); /* perturbation mode, number of nodes per period */
#ifdef MHD
    ...
    for (k=ks; k<=ke; k++) {
      for (j=js; j<=je; j++) {
        for (i=is; i<=ie; i++) {
          ...
          pGrid->U[k][j][i].M2 = amp*sin( permode *2.*PI*x1)*exp(-(x2*x2)/(sigma*sigma));
          pGrid->U[k][j][i].M3 = 0.0;
#ifdef MHD
          pGrid->B1i[k][j][i] = b0;
          pGrid->U[k][j][i].B1c = b0;
